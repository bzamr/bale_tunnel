...
/// Reads from the SOCKS5 socket and sends upstream chunks to the server.
            ...
            res = timeout(inactivity_timeout, stream.read_buf(&mut buffer)) => {
                match res {
                    Ok(Ok(0)) => {
                        ...
                        info!("Upstream EOF for session {session_id}, flushing remaining buffer");
                        if !buffer.is_empty() {
                    ...
                    Err(_elapsed) => {
                        ...
                        if !buffer.is_empty() {
                            info!("Flushing {} bytes due to inactivity (upstream)", buffer.len());
                            let data = buffer.split().to_vec();
                            let packet = build_packet(session_id, seq, &data);
                            session_mgr.send_upstream_chunk(session_id, packet).await?;
                            seq += 1;
...
/// Receives downstream chunks from the channel and writes them to the SOCKS5 socket.
        ...
        tokio::select! {
            Some((hint_seq, packet)) = rx_data.recv() => {
                ...
                let Some(payload) = payload else {
                    // decompress failed — cancel_token was already triggered
                    break;
                };
            ...
            () = cancel_token.cancelled() => {
                info!("Downstream end signal for session {session_id}, closing");
                break;
            }
            () = tokio::time::sleep_until(last_activity + Duration::from_secs(30)) => {
                warn!("No downstream data for 30 seconds, closing session {session_id}");
                cancel_token.cancel();
                break;
            }
        }
